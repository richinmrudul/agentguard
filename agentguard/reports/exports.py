...
SECRET_PATTERNS = [KEYED_CREDENTIAL_PATTERN, *TOKEN_SHAPE_PATTERNS]
...
def render_junit(
    ...
    *,
    ...
    root = ElementTree.Element(
        "testsuites",
        {
            "name": _junit_xml_text(_sanitize(suite_name)),
            "tests": str(len(cases)),
    ...
    for case in sorted(cases, key=lambda item: (item.classname, item.name)):
        testcase = ElementTree.SubElement(
            ...
            "testcase",
            {
                "classname": _junit_xml_text(_sanitize(case.classname)),
                "name": _junit_xml_text(_sanitize(case.name)),
    ...
    ElementTree.SubElement(suite, "system-out").text = (
        _junit_xml_text(_sanitize(f"Generated by {tool_name} {__version__}"))
    )
...
def _sarif_rule(
    ...
    return {
        "id": _sanitize(rule_id),
        "name": _sanitize(first.rule_name),
        "shortDescription": {"text": _sanitize(first.rule_name)},
        "fullDescription": {
            "text": f"AgentGuard policy check: {_sanitize(first.rule_name)}."
        },
        "defaultConfiguration": {"level": severity},
        "properties": {
            "agentguardSeverity": _sanitize(first.severity),
...
def _sarif_result(finding: ExportFinding) -> dict[str, Any]:
    result: dict[str, Any] = {
        "ruleId": _sanitize(finding.rule_id),
        ...
        "properties": {
            "agentguard": {
                ...
                "reportPath": _sanitize(finding.report_path),
                "runId": _sanitize(finding.run_id),
                "taskId": _sanitize(finding.task_id),
                "agent": _sanitize(finding.agent),
                "sourceType": _sanitize(finding.source_type),
...
def _fingerprint_material(finding: ExportFinding) -> str:
    material = {
        "version": 1,
        "rule_id": _sanitize(finding.rule_id),
        "task_id": _sanitize(finding.task_id),
        "agent": _sanitize(finding.agent),
...
def _result_message(finding: ExportFinding) -> str:
    ...
    parts = [f"{_sanitize(finding.rule_name)} {state}: {_sanitize(finding.message)}"]
    if finding.evidence:
...
def _sanitize(value: object) -> str:
    text = redact_credentials(value, marker="<redacted>")
...
def _system_out(
    *,
    ...
    if paths:
        lines.append(
            "paths: "
            + ", ".join(_normalize_repo_path(path) or _sanitize(path) for path in paths)
        )
    if evidence:
