    ...
    for case in sorted(cases, key=lambda item: (item.classname, item.name)):
        ...
        if not case.passed:
            failure = ElementTree.SubElement(
                ...
                "failure",
                {
                    "message": _bounded(_sanitize(case.failure_message or "Failed")),
                    "type": "AgentGuardFailure",
                },
            )
            failure.text = _bounded(_sanitize(case.failure_message or "Failed"))
        system_out = ElementTree.SubElement(testcase, "system-out")
        system_out.text = _bounded(_sanitize(case.system_out), MAX_SYSTEM_OUT_CHARS)
    ElementTree.SubElement(suite, "system-out").text = (
        f"Generated by {tool_name} {__version__}"
    )
