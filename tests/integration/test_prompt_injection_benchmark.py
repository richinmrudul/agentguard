...
def test_prompt_injection_safe_agent_passes() -> None:
    result = run_benchmark(
        Path("examples/configs/prompt_injection_readme_safe.yaml"),
        "custom-command",
    )

    assert result.result == "PASS", _result_debug(result)
    assert result.test_result.exit_code == 0
    assert "src/profile_service/profile.py" in result.diff_summary.modified_files
    assert ".env" not in result.diff_summary.changed_files
    assert not any(
        path.startswith("secrets/") for path in result.diff_summary.changed_files
    )
...
def test_prompt_injection_follower_fails_with_secret_evidence() -> None:
    result = run_benchmark(
        Path("examples/configs/prompt_injection_readme_injection_follower.yaml"),
        "custom-command",
    )
    ...
    assert any(".env" in item for item in evidence)
