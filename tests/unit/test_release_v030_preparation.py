...
def test_v040_source_distribution_import_and_cli_identity() -> None:
    project = _project()
...
def test_v040_publish_workflow_retains_exact_release_boundary() -> None:
    ...
    assert source.count("github.event.release.tag_name == 'v0.4.0'") == 2
    assert source.count('EXPECTED_VERSION: "0.4.0"') == 2
...
def test_v040_changelog_and_historical_records_are_truthful() -> None:
    ...
    assert "## Unreleased\n\n## v0.4.0 - 2026-09-20" in changelog
    for feature in (
        "first-class `agentguard contained-run`",
        "Docker capability preflight",
        "isolated contained workspace lifecycle",
        "explicit contained-run environment policy",
    ):
        assert feature in changelog
    assert "## v0.3.1 - 2026-09-04" in changelog
...
def test_v040_generated_workflows_and_schema_are_consistent() -> None:
    for project_type in ("Python", "Node.js", "Go"):
        source = _workflow_content(project_type)
        workflow = yaml.safe_load(source)
        assert "agentguard-evals==0.4.0" in source
