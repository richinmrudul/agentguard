...
def test_junit_render_replaces_every_xml_illegal_c0_control() -> None:
    ...
    assert len(illegal_c0) == 29
    source = f"C0-START{illegal_c0}C0-END"

    xml = report_exports.render_junit(
        [
            report_exports.ExportTestCase(
                classname=f"class-{source}",
                name=f"name-{source}",
                passed=False,
                failure_message=f"failure-{source}",
                system_out=f"output-{source}",
            )
        ],
        tool_name=f"Tool-{source}",
        suite_name=f"Suite-{source}",
    )

    ElementTree.fromstring(xml)
    for control in illegal_c0:
        assert control not in xml
    assert "C0-START" in xml
    assert "C0-END" in xml
    assert xml.count("\ufffd") >= len(illegal_c0) * 5


def test_junit_render_replaces_noncharacters_and_unpaired_surrogates() -> None:
    legal_replacement = "\ufffd"
    noncharacters = "\ufffe\uffff"
    unpaired_surrogates = "\ud800\udfff"
    ...
    canary = f"{legal_replacement}{noncharacters}{unpaired_surrogates}"
    ...
    for illegal_char in noncharacters + unpaired_surrogates:
        assert illegal_char not in xml
    assert "Suite\ufffd\ufffd\ufffd\ufffd\ufffd" in xml
    ...
    assert (
        failure.text is not None
        and "failure\ufffd\ufffd\ufffd\ufffd\ufffd<tag" in failure.text
    )
    assert dangerous_xml in failure.text
    assert case_output.text is not None and len(case_output.text) <= 1600
    assert suite_output.text == (
        f"Generated by Tool\ufffd\ufffd\ufffd\ufffd\ufffd{dangerous_xml} "
        f"{report_exports.__version__}"
    )
