...
v0.4.0 uses this process; after publication, record its immutable release
evidence under `docs/results/`. Historical v0.3.1 evidence remains in
[`results/release-v0.3.1.md`](results/release-v0.3.1.md), historical v0.3.0
evidence remains in
...
- [ ] Wheel is `agentguard_evals-0.4.0-py3-none-any.whl`.
- [ ] Source distribution is `agentguard_evals-0.4.0.tar.gz`.
- [ ] `pyproject.toml`, `agentguard.__version__`, installed
  `agentguard --version`, wheel metadata, and sdist metadata all equal `0.4.0`.
...
- [ ] Tag `v0.4.0` points to the reviewed release commit.
...
- [ ] The environment allows only the exact proposed release tag. Version
  0.4.0 uses the selected tag rule `v0.4.0`.
...
- [ ] Create and push annotated tag `v0.4.0`; do not create tags automatically.
